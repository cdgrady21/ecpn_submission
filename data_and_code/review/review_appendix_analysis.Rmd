...
# robustness checks
## make a version thats all NA and a version thats all 0
ag.df$qip_group.borehole_use_end_NA <- ifelse(ag.df$qip_group.borehole_use_end %in% 0, NA, ag.df$qip_group.borehole_use_end)
ag.df$qip_group.borehole_use_end_0 <- ifelse(is.na(ag.df$qip_group.borehole_use_end), 0, ag.df$qip_group.borehole_use_end)

############
# must do effect of borehole aware/use among subset of treated comms/ppl bcuz TR and borehole use/aware perfectly correlated
...
rownames(all_coefficients) <- c("intercept", "estimate")

mean(all_coefficients["estimate",colnames(all_coefficients)[grepl("attitude", colnames(all_coefficients))]])

all_pvalues <- sapply(qipList, function(x) x$p.value)
rownames(all_pvalues) <- c("intercept", "variable")
mean(all_pvalues["variable",])
mean(all_pvalues["variable",colnames(all_pvalues)[grepl("borehole_use", colnames(all_pvalues))]])

summary(all_pvalues["variable",])
...
ggplot(ag.df[ag.df$treatment %in% 1,], aes(x = qip_group.borehole_use_end, y = attitude_cw, color = psu)) +
  geom_point() +
  geom_text(aes(label = comm), vjust = -0.5) +
  geom_abline(slope=tidy(lm1)[2, 2],
              intercept=tidy(lm1)[1, 2],
              color="black",
              size=1) +
  theme_bw()

qipList$attitude_cw$term_use

######################################
# when it was two functions 
######################################################
# loop over outcome list (from g-aggAnalysis-mainOutcomes.Rmd) and save
outcome_list <- c("attitude", "in", 'contactOnly',
                  'rMean', "end_exp",
...
{
  # boreholes
  thelm1 <- lm_robust(ag.df[ag.df$treatment %in% 1,outcome_list_qip[i]]~
                       ag.df[ag.df$treatment %in% 1,"qip_group.borehole_use_end"],
                            clusters = psu, data=ag.df[ag.df$treatment %in% 1,])
  thelm2 <- lm_robust(ag.df[ag.df$treatment %in% 1,outcome_list_qip[i]]~
                       ag.df[ag.df$treatment %in% 1,"qip_group.borehole_aware_end"],
                            clusters = psu, data=ag.df[ag.df$treatment %in% 1,])
  # qips
  thelm3 <- lm_robust(ag.df[ag.df$treatment %in% 1,outcome_list_qip[i]]~
                       ag.df[ag.df$treatment %in% 1,"qip_ben_end"],
                            clusters = psu, data=ag.df[ag.df$treatment %in% 1,])
  thelm4 <- lm_robust(ag.df[ag.df$treatment %in% 1,outcome_list_qip[i]]~
                       ag.df[ag.df$treatment %in% 1,"qip_aware_end"],
                            clusters = psu, data=ag.df[ag.df$treatment %in% 1,])
  
  lm1_res <- tidy(thelm1)[, c(1,2,5)]
  lm2_res <- tidy(thelm2)[, c(1,2,5)]
  names(lm1_res) <- paste0(names(lm1_res), "_bore_use")
  names(lm2_res) <- paste0(names(lm2_res), "_bore_aware")
  
  lm3_res <- tidy(thelm3)[, c(1,2,5)]
  lm4_res <- tidy(thelm4)[, c(1,2,5)]
  names(lm3_res) <- paste0(names(lm3_res), "_qip_ben")
  names(lm4_res) <- paste0(names(lm4_res), "_qip_aware")
  
  
  qipList[[i]] <- cbind(lm1_res, lm2_res, lm3_res, lm4_res)
}
names(qipList) <- outcome_list_qip
save(qipList, file="qip_list.Rda")

# create graphs, save graphs
## graph fun
qip_graph_fun <- function(ben_var, out_var, title, xlab, ylab)
{
  ggplot(ag.df[ag.df$treatment %in% 1,], aes(x = ag.df[ag.df$treatment %in% 1,ben_var], 
                                             y = ag.df[ag.df$treatment %in% 1,out_var], 
                                             color = psu)) +
...
#qip_graph_fun(ben_var="qip_group.borehole_use_end", out_var="attitude_cw", 
#              title="Borehole use and attitudes", xlab="borehole use", ylab="attitude change")
ben_vars <- c("qip_group.borehole_use_end", "qip_group.borehole_aware_end",
              "qip_ben_end", "qip_aware_end")
ben_out_df <- expand.grid(ben_vars = ben_vars,
                    out_vars = outcome_list_qip)
