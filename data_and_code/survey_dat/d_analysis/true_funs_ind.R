...
###########
# committee vs control and non-committee vs control
## differencing
comp.fun1 <- function(var, nsims)
{
  ...
  # I think correct thing is: (1) for treatment shuffle PSU-level treatment within state, (2) for committee shuffle committee within Community for the comm/non comparison.  To avoid committee people in non-treated areas, need to make "committee" column for "newtr" the same as committee for "treatment" before shuffling.
  for(i in 1:nsims){
    # randomly select for treatment PSUs within state
    rand.nas <- sample(unique(panel.df$psu[panel.df$state %in% "nas"]), size=6)
    rand.ben <- sample(unique(panel.df$psu[panel.df$state %in% "ben"]), size=4)
  ...
  # now it's: "how many times are these rand coefs greater than or equal to what we observed?"
  thep_nonpart <- mean(rand.coefs[,'nonpart']>obs.coef_nonpart)
  thep_part <- mean(rand.coefs[,'part']>obs.coef_part)
  thedf_nonpart <- data.frame(thecoef=obs.coef_nonpart, truep=thep_nonpart)
  thedf_part <- data.frame(thecoef=obs.coef_part, truep=thep_part)
  rownames(thedf_nonpart) <- paste0(var, "-", "nonpart")
  rownames(thedf_part) <- paste0(var, "-", "part")
  thedf <- rbind(thedf_nonpart, thedf_part)
  return(thedf)
}
#comp.fun1("attitude_cw",nsims=1000)

# Controlling-for
comp.fun <- function(var, nsims)
{
  ...
  rand.coefs <- matrix(data=NA, nrow=nsims, ncol=2)
  colnames(rand.coefs) <- c("nonpart", "part")
  # I think correct thing is to: (1) for treatment shuffle PSU-level treatment within state, (2) for committee shuffle committee within Community for the comm/non comparison.  To avoid committee people in non-treated areas, need to make "committee" column for "newtr" the same as committee for "treatment" before shuffling.
  for(i in 1:nsims){
    # randomly select for treatment PSUs within state
    rand.nas <- sample(unique(panel.df$psu[panel.df$state %in% "nas"]), size=6)
    rand.ben <- sample(unique(panel.df$psu[panel.df$state %in% "ben"]), size=4)
...
strat.fun <- function(var, dat=panel.df){
  ...
  thebal_part <- thediff_part-thesd
  thebal_nonpart <- thediff_nonpart-thesd
  if(thebal_part<0 & thebal_nonpart<0){
    return("Controlling-for, comp.fun")
  }
  if(thebal_part>=0 | thebal_nonpart>=0){
    return("Difference, comp.fun1")
  }
  else(return("Fail"))
}
