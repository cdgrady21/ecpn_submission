...
# Orig code from Nuole

```{r}
# From Nuole
plot_hyptab_taxout <- ggplot(hyptab_taxout, aes(y=estimate, x = reorder(outcome, -order))) +
  geom_hline(aes(yintercept = 0), linetype="dashed", color="gray")+
  geom_point(aes(shape=estimator_type), size=4) +
  scale_shape_manual(values=c("lin" = 19, "robust" = 17))+
