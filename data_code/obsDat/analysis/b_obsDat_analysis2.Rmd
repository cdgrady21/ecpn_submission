...
This is an analysis of the market and event observational data.  The original outcomes are the raw number of people attending the market/event or engaging in some behavior at the market/event.  The outcomes are non-normal and skewed.  

As a result of this skew, outcomes are coded in multiple ways:

- raw number
- (dense) rank rescaled number

The outcomes index is also created in multiple ways:

- additive index
- others?

Outcomes are also estimated in multiple ways:

- OLS with time/unit FEs
- quantile regression
- robust regression.

And the analysis is done in multiple ways: 

- Change score between T1 and T2 (collapse all time=T1 outcomes to "T1" and all time=T2 outcomes to "T2")
- panel analysis ()
