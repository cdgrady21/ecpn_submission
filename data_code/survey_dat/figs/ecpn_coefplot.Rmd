...
load("../../obsDat/figs/obsDat_fig_dat.rda")
...
```{r}
outcome <- ag.coefs
outcome <- outcome[grepl("Trust|contact|in_cw|end_exp|rMean", outcome$var),]
...
outcome$var <- c("Self-reported Attitudes", "Self-reported Contact", 
                 ...
                 "Percent Experiment", "Endorsement Experiment")
outcome$outcome_type <- c("atts", "con", "in", "con", "atts")
...
```{r}
outcome2$outcome_type <- outcome2$df
outcome2$df <- "obs"
...
svyObs_outcomes$var <- 
  factor(svyObs_outcomes$var, 
         levels = c("Perceptions of Insecurity",
                    "Endorsement Experiment",
                    "Self-reported Attitudes", 
                    "Outgroup Event Attendance",
                    "Farmers in Market","Pastoralists in Market",
                    "Percent Experiment",
                    "Self-reported Contact"))
svyObs_outcomes$hypothesis <- c("Attitudes", "Contact", "Insecurity", "Contact",
                                "Attitudes", "Contact", "Contact", "Contact")

svyObs_coefplot <- ggplot(svyObs_outcomes, aes(y=coef_sd, x = var)) +
  geom_hline(aes(yintercept = 0), linetype="dashed", color="gray")+
  geom_point(aes(shape=hypothesis, size=4)) +
  scale_shape_manual(values=c("Attitudes"=19, "Contact"=17, "Insecurity"=18))+
  geom_errorbar(aes(ymin=ll_sd, ymax=ul_sd, width=0, linetype=df)) +
  scale_linetype_manual(values = c("dotted", "solid")) +
  labs(x = "Outcome", y = "Est. Effect Size and 95% CI") +
  scale_y_continuous(limits=c(-1, 2),
                     n.breaks=7)+
...
pdf(file="svyObs_coefplot.pdf",bg="transparent", width=10, height=7)
svyObs_coefplot
dev.off()

png(file="svyObs_coefplot.png",bg="transparent")
svyObs_coefplot
dev.off()
