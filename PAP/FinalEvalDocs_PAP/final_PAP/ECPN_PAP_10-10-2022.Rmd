...
We also measure placebo outcomes: attitudes towards violence, attitudes towards other religious groups, and radio listening.  The primary purpose of these placebo outcomes is to help us rule out other explanations for treatment-control differences, like greater social desirability bias in the treatment group. Attitudes about violence are a good candidate for a “placebo outcome” because intergroup contact should not affect general attitudes about violence, but respondents may feel social pressure to answer violence questions in a desirable way. Attitudes towards people from other religions should not be affected by ECPN but would signal either social desirability bias or a general increase in tolerance towards outgroups.  Radio listening should also not be affected by ECPN but is less relevant for how self-reported attitudes change from baseline to endline.
...
$$ Y_{i,j} = \beta_0 + \beta_1 Z_{i,j} + X_{i,j} + D_{j} + \epsilon_{i,j} $$
...
$$ Y_{i,j,k} = \beta_0 + \beta_1 Z_{i,j,k} * \beta_2 T_{i,j,k} + X_{j} + D_{k} + \epsilon_{i,j, k} $$
...
$$ Y_{i,j} = \beta_0 + \beta_1 Z_{i,j} + D_{j} + \epsilon_{i,j} $$
...
$$ Y_{i,j} = \beta_0 + \beta_1 P_{i,j} + \beta_2 N_{i,j} + X_{i,j} + D_{j} + \epsilon_{i,j} $$
...
$$ Y_{i,j} = \beta_0 + \beta_1 P_{i,j} + \beta_2 N_{i,j} + D_{j} + \epsilon_{i,j} $$
